  ...
  <NuxtLink :to="getLink()">
    <div class="nyhedsCard p-[1rem]">
      <div class="relative overflow-hidden">
        ...
        <!-- Gradient overlay -->
        <div
          class="absolute bottom-0 left-0 w-full h-24 bg-gradient-to-t from-black/70 to-transparent pointer-events-none"
        ></div>

        <p
          class="absolute bottom-0 left-0 text-neutral-6 font-anton text-[2.5rem] ml-[1rem] z-10"
        >
          {{ dato }}
        </p>
      </div>
      <div class="flex mt-[1rem]">
        <div>
          <h4 class="font-anton text-[2rem]">{{ post.title.rendered }}</h4>
        </div>
        <div>
          <p
            class="border-solid border-2 p-2 border-neutral-1 rounded-full ml-[0.5rem]"
          >
            {{ post.acf.eventtype[0]?.name }}
          </p>
        </div>
      </div>
    </div>
